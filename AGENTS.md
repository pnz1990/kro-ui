...
PR_LABEL:       kro-ui
REPORT_ISSUE:   637
REPORT_URL:     https://github.com/pnz1990/kro-ui/issues/637
BOARD_URL:      ""
...
| Serving over-sized images at wrong display dimensions | #661 | `web/public/logo.png` and `favicon.png` were 1984×2138px / 4.1MB served at 24×24 and 32×32. The Lighthouse CI scored 41/100 (threshold 50) — LCP render delay 36.8s from loading 8.2MB of unoptimized PNG. Always ensure `web/public/` image files match the display size; regenerate as small PNGs or use SVG. Source PNG at repo root may remain at full resolution. |
| Polling components updating UI without `aria-live` announcements | #668 | Components that poll (e.g. `usePolling`) update the DOM silently; screen readers receive no feedback on state changes. WCAG 2.1 SC 4.1.3 (Status Messages) requires status updates to be programmatically determinable. Add `aria-live="polite"` region + `prevRef` (useRef) to announce only on transition (prev ≠ current), not every poll cycle. |
| Missing skip-to-main-content link in Layout | #667 | Without a skip-nav link, keyboard and screen reader users must Tab through the entire navigation bar on every page load. WCAG 2.1 SC 2.4.1 (Bypass Blocks) requires a skip mechanism. Add `<a href="#main-content" className="layout__skip-link">` as the first focusable element; add `id="main-content"` to `<main>`; use `.sr-only` CSS for visually hidden until focused. |
