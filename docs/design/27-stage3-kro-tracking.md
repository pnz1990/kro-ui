...
- 🔲 27.37 — Honesty: `metrics.md` column schema is out of sync with the SM `[AI-STEP]` row format — `docs/aide/metrics.md` has a 10-column header (`Date | Batch | prs_merged | predicted_prs | needs_human | ci_red_hours | skills_count | skills_delta | todo_shipped | notes`) but the SM `[AI-STEP]` comment in `~/.otherness/agents/phases/sm.md` §4b specifies a 15-column format (`$DATE | $BATCH | $MERGED | $NEEDS_HUMAN | 0 | $SKILLS | $TODO_SHIPPED | ~Xmin | $VISION_PRS | $SESSION_OUTCOME | $ARCH_CONVERGENCE | $SIM_FLOOR_DELTA | $QUEUE_GUARD_FIRES | $MEANINGFUL_PRS | <notes>`); rows written by the SM will never align with the header, making the table unreadable by automated tooling; 27.35 addresses the missing `predicted_prs`/`skills_delta` writes but does not address the underlying schema divergence; the fix requires choosing one canonical schema and updating both `metrics.md` header and the SM `[AI-STEP]` format comment to match — otherwise any script parsing the metrics table by column index will silently read the wrong field
- 🔲 27.38 — Self-improvement: COORD learn trigger ignores `skills_delta` stagnation — COORD §1g triggers `/otherness.learn` when `queue < 5 items OR days_since_learn >= 3 days`; item 27.24 specifies that learn should also trigger when `skills_count` has not increased in 5 consecutive batches, but this condition is not in `coord.md`; the current trigger fires on schedule/queue pressure regardless of whether the skills library is growing; a learn session that runs every 3 days but adds 0 skills satisfies the timer condition while the monoculture gap (27.25) remains uncorrected; add a `skills_delta=0` stagnation check to COORD §1g that reads the last 5 rows of `metrics.md`, counts consecutive zero-delta rows, and forces a learn session targeted at the top 3 most-commented open issues when the stagnation threshold is exceeded
