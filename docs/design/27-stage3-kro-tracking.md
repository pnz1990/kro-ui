...
- 🔲 27.28 — Loop honesty: simulation is entirely non-functional — `sm.md §4d` references `sim-prediction.json`, `sim-params.json`, and `scripts/calibrate.py`, but the `_state` branch contains only `state.json`, `scripts/calibrate.py` does not exist in the repo, and `sim-prediction.json` has never been written; the SM's simulation step silently fails every batch (non-fatal error path) and the agent loop never receives a floor/ceiling prediction; the section on "simulation divergence" in `sm.md` is dead code; fix by either (a) implementing the missing `scripts/simulate.py` and `scripts/calibrate.py` with real calibration logic, or (b) explicitly removing the simulation references and replacing with a simpler batch-count spiral detector; the current state — a described-but-broken simulation — is dishonest because it implies a feedback loop that does not exist
- 🔲 27.29 — Visibility: 27.26 is underspecified — item 27.26 says "add a `docs/aide/loop-health.md` file that the SM updates on every batch" but the SM phase (`phases/sm.md`) has no step that writes to `docs/aide/loop-health.md`; the file will never exist unless the SM is explicitly programmed to create and update it; `vibe-vision-auto` cannot promote 27.26 to ✅ without evidence the SM is writing the file; add a concrete implementation task: in `sm.md §4f` (the condensed report section), after posting the issue comment, also write `docs/aide/loop-health.md` with a 3-line quick-glance header followed by the full metrics snippet; also update the issue #439 body (one-time) with a pinned link to the file; without this implementation step 27.26 remains indefinitely 🔲
- 🔲 27.30 — Reliability: session item limit may prevent meaningful PRs — `otherness-config.yaml` sets `session_item_limit: 3`; each session completes at most 3 queue items; if all 3 items are hygiene/chore (test coverage, doc fixes), the session ships 0 design-doc-backed PRs and is classified CHORE-ONLY; the SM correctly posts AMBER in this case (added in recent standalon.md update) but the root cause — hygiene items crowding out feature items — is not addressed; add a COORD rule: hygiene items may not occupy more than 1 of the 3 session slots unless the feature queue is genuinely empty; record which slot each item occupies in `state.json` so the SM can enforce the ratio; a loop that is reliably shipping ≥1 feature PR per session requires both the SM AMBER signal AND the COORD slot reservation
