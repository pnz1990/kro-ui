...
- ✅ 27.13 — DAG scale guard: RGDs with >100 nodes show a text-mode node list by default instead of a dense SVG that locks up the browser; a "Show graph (N nodes — may be slow)" toggle allows opt-in to the full render; nodes are grouped by type with conditional badges; DAGScaleGuard wraps DAGGraph and StaticChainDAG. (PR #TBD, spec issue-575)
- ✅ 27.14 — Frontend code splitting: route-based React.lazy + Suspense per route reduces initial bundle from ~521KB to vendor + entry chunks (~150KB gzipped); perf.yml threshold raised from 50 to 70; PageLoader shimmer fallback renders within Layout shell while chunks load. (PR #TBD, spec issue-576)
...
- ✅ 27.26 — Visibility: single-page loop health dashboard — `docs/aide/loop-health.md` created by vibe-vision-auto scan (2026-04-21); contains quick-glance status, last feature PR, next priority, E2E anchor score, open loop health gaps, and metrics summary; SM must update it after every batch. (vibe-vision-auto, 2026-04-21)
...
- 🔲 27.32 — Metrics staleness: SM not updating `docs/aide/metrics.md` within the session — today's sessions (PRs #596–#612, ~17 PRs including feat PRs for issue-535, #536, #537, #538, #539, #576) are not recorded in the metrics table which ends at batch 51 (2026-04-20); the `predicted_prs` and `skills_delta` columns were added to the header row today but every data row still shows `-` for both; the SM phase instruction to update `metrics.md` is present in `standalone.md` but the agent skips it when sessions are feature-focused; add an explicit post-batch metrics write to the SM checklist that fires before the health signal comment — if the metrics row was not written, the health signal MUST include a `⚠️ metrics not updated` flag; metrics staleness erodes the simulation's ability to detect housekeeping spirals and prediction accuracy
- ✅ 27.33 — Visibility: vibe-vision-auto scan creates `docs/aide/loop-health.md` proactively — created by this scan run (2026-04-21); the file now exists at `docs/aide/loop-health.md` with quick-glance status, last feature PR, next priority, and open gaps table.
- 🔲 27.34 — Reliability: COORD phase does not fill `predicted_prs` column — the column definition was added to `docs/aide/metrics.md` header on 2026-04-21 as part of implementing 27.30, but the COORD phase (which runs at session start to select items from the queue) does not actually write a metrics row with `predicted_prs` before handing off to ENG; when the SM writes the batch-end metrics row, there is no `predicted_prs` value to record (the COORD hasn't filed one); the implementation of 27.30 is incomplete: add an explicit COORD checklist step that estimates the number of shippable PRs for this session (based on queue items and their size labels) and appends a partial metrics row to `docs/aide/metrics.md` before the ENG phase begins — the SM fills in actuals at session end
- 🔲 27.35 — Self-improvement: skill bootstrap from repeated retries — the most common source of rework in kro-ui sessions is E2E journey syntax errors (missing `})`, locator ambiguity, `waitForTimeout` misuse, SPA HTTP-200 pitfall); these patterns appear in AGENTS.md §Known anti-patterns E2E rows but are not formalized as a callable skill in the otherness skills library; when the loop retries an E2E-related PR more than once, it is re-learning something it has already learned; add a `/otherness.learn` invocation after any PR that required 2+ retry commits due to E2E failures, targeted at extracting a reusable E2E skill; the skill bootstrap should be automatic, not manual — if the loop can detect "I retried this because of an E2E syntax error," it should also be able to say "I'll create a skill so I don't do this again"
