...
- 🔲 27.22 — Meaningful-PR session quality gate: the scheduled loop currently ships housekeeping PRs (dependency bumps, doc fixes, metric updates) that count as "work done" but do not advance the product; add a session quality gate in `otherness-scheduled.yml` that detects housekeeping-only sessions (heuristic: PR title prefix is `chore`/`fix(workflow)`/`chore(deps)` with no `feat` PR in the same batch) and posts a `[WARN] zero-feature session` annotation to issue #439; this makes empty sessions visible without requiring human review of the log; the gate should trigger a COORD priority escalation on three consecutive zero-feature sessions — the system should diagnose its own stall, not wait for the human to notice
- 🔲 27.23 — SM health signal calibration: the SM currently posts `GREEN` / `YELLOW` / `RED` health signals to issue #439 but the calibration is purely heuristic (no definition of what triggers each state); define concrete thresholds in `otherness-config.yaml` or a design doc: GREEN = ≥1 feat PR merged in last 5 sessions; YELLOW = 0 feat PRs in last 5 sessions but no CI failures; RED = CI red or 0 feat PRs in last 10 sessions; without explicit thresholds the signal is meaningless — a loop that only ships chores will report GREEN indefinitely because no threshold is violated
- 🔲 27.24 — Single-page system health dashboard: a human looking at GitHub right now cannot quickly tell if the system is healthy, what it shipped today, and whether it is moving toward the vision or spinning in circles; the report issue (#439) has 289+ comments that require scrolling to interpret; replace the raw comment stream with a pinned comment that the SM rewrites each session: a table showing last-7-sessions (date, feat PRs, chore PRs, CI status, health signal) plus a single verdict line ("advancing" / "stalled" / "degraded"); this is the single-page health dashboard the operator needs — it costs one extra gh comment rewrite per session and removes the need to read 100+ comment history to assess system state
- 🔲 27.25 — Pressure context self-rewrite: the vision pressure context in `otherness-scheduled.yml` is static — it was written once and never updated as the system makes progress; Scan 5 in `vibe-vision-auto` detects when ≥60% of pressure keywords appear in recent PRs but only adds a reminder item; extend the scan to actually propose a rewritten pressure block (as a diff comment on issue #439 or a PR to `otherness-scheduled.yml`) so the human can approve the new pressure with a single click; as long as the pressure block stays unchanged, the system re-evaluates against goals it has already met, which wastes each session's analysis budget on already-shipped items
- 🔲 27.26 — Onboarding automation quality gate: adding a new project to otherness today still requires manually editing at least 3 files (`otherness-config.yaml`, `.github/workflows/otherness-scheduled.yml`, the first design doc); `/otherness.onboard` produces skeleton docs but the GitHub Projects IDs, field IDs, and option IDs must be filled in by the human; add a `scripts/onboard-verify.sh` that validates every required field in `otherness-config.yaml` is non-placeholder, all referenced GitHub Projects IDs resolve via the API, and the scheduled workflow has the correct repo reference; post the verification result to the REPORT_ISSUE on first run so onboarding gaps are immediately visible; without this, a misconfigured project silently produces sessions that fail or produce no output
