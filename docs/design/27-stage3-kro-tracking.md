...
- 🔲 27.28 — Reliability: silent session failure detection — when an otherness session crashes, times out, or runs out of context before completing the SM phase, the report issue receives no final status comment and the metrics table is not updated; a human checking tomorrow sees no evidence anything ran and cannot distinguish "ran and shipped nothing" from "did not run at all"; add a session heartbeat: the scheduled workflow must post an explicit "session complete" or "session failed" comment to issue #439 in a GitHub Actions `always()` step, regardless of whether the session step succeeded or exited early; without this, every silent failure looks identical to a successful empty batch from the outside
- 🔲 27.29 — Reliability: queue-empty guard — when the open GitHub issue queue has no `feat:` or `fix:` items labeled for the agent to pick up, the session currently stalls (or ships a housekeeping micro-PR to avoid appearing idle); this is a coverage/reliability issue: a truly reliable system must generate its own next work item when the queue is empty rather than waiting for human triage; add a rule to the SM phase: if `gh issue list --label kro-ui --state open` returns 0 items before COORD runs, the SM must open a new issue from the `🔲 Future` queue in the design docs before handing off to COORD; the design docs are the backlog; the SM must know how to drain them into the issue queue autonomously
- 🔲 27.30 — Loop honesty: simulation predictions must visibly change agent behavior — the otherness system includes a simulation/prediction step (SM §4 health signal) but the predicted outcomes are never compared against actual outcomes in a subsequent batch; a prediction that "this batch will ship 2 features" followed by "batch shipped 0 features" should trigger a self-correction note; add a `predicted_prs` field to the metrics table and a SM check that compares the previous batch's prediction against the actual result; if accuracy is below 50% for 3 consecutive batches, the SM must flag this as a calibration failure and adjust its prediction model (e.g. halve the predicted count); predictions that never influence behavior are theater, not signal
- 🔲 27.31 — Loop honesty: metrics-to-action pipeline — `docs/aide/metrics.md` is updated each batch but the data in it is never consumed by any automated rule; `ci_red_hours=0` for all 9 rows, `skills_count=12` for 7 rows, `needs_human=0` for all rows — these are flat lines that suggest the metrics are being collected but no threshold is ever crossed; add at least 3 explicit SM rules that fire when metrics cross thresholds: (1) if `skills_count` unchanged for 5 batches → auto-invoke `/otherness.learn`; (2) if `ci_red_hours > 2` in any batch → SM sets health AMBER and opens a `fix: CI` issue before COORD; (3) if `prs_merged` drops to 0 two batches in a row → SM posts RED and escalates to the report issue with a call for human review; metrics that drive zero decisions are not worth collecting
- 🔲 27.32 — Visibility: report issue comment verbosity cap — issue #439 receives a new comment every hourly batch; each comment contains full metrics tables, session hashes, CI status lines, and debug-level detail; after 600+ PRs and dozens of batches, the comment stream is several hundred entries long and effectively unreadable for a human doing a quick health check; add a MAX_COMMENT_LENGTH cap (suggested: 400 characters) to the SM batch summary comment, with a link to `docs/aide/loop-health.md` for the full detail; the human-visible comment should answer exactly: "✅/⚠️/🔴 status — last feature shipped: X — next priority: Y — [full report](link)"; verbose technical detail belongs in the linked doc, not in the issue comment stream
- 🔲 27.33 — Design doc numbering collision fix: doc 27 §Future has items numbered 27.22–27.24 for *product* items (release.yml permissions, SECURITY.md post-donation, community outreach) and the same numbers 27.22–27.24 reused in the *System Loop Health* section for loop process items; this causes ambiguity when either section is referenced in PRs, commit messages, or issues; renumber the System Loop Health items to 27.SL-01 through 27.SL-0N (or use a `27.L` prefix) to distinguish them from product roadmap items; all cross-references in vision.md and metrics.md must be updated accordingly
