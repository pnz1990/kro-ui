...
- 🔲 27.15 — release.yml stale helm/ reference: the `Update and push Helm chart` step in `.github/workflows/release.yml` references `helm/kro-ui` which was removed in v0.9.4; the next `v*` tag push will fail at this step; remove the entire `Update and push Helm chart` step from release.yml before cutting v0.10.0
