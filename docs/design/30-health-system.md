...
- 🔲 Health system: SRE dashboard health trends over time — the SRE dashboard (PR #405) shows current state but has no historical trend; an operator cannot tell whether the cluster has been getting healthier or more degraded over the past hour; add a spark-line chart per health state (ready/degraded/error counts) using data sampled at each 5s poll cycle, retained in-memory for the current session (max 720 points = 1 hour); no server-side storage required; the chart must update live and degrade gracefully to a single data point when only one poll has run; this directly addresses the "metrics collected but not acted on" pressure lens — poll data is already available, it is just not visualized over time (2026-04)
- 🔲 Health system: health state export — an operator debugging a cluster incident needs to share the current health snapshot with their team; the only export today is instance YAML (copy button, PR #277); add a "Copy health snapshot" button to the SRE dashboard that produces a structured JSON blob with: health state per RGD, top-5 error messages, cluster context name, and a timestamp; this can be pasted into a Slack message or incident ticket without requiring the recipient to have kubectl access; the JSON format must be stable enough that future versions can parse it (2026-04)
