...
- ✅ 30.2 — Cross-instance error pattern correlation: `aggregateTopMessages()` pure function added to ErrorsTab; renders a "Top error messages" panel above the error groups showing `{count}× {message}` sorted by frequency; only messages appearing in >1 instance are shown; 7 unit tests; no new backend required (spec issue-775, 🔲→✅ 2026-04)
