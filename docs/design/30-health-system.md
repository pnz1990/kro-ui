...
- 🔲 Health snapshot clipboard export: "Copy health snapshot" button on Overview SRE dashboard — serializes current fleet health state (per-cluster counts, error patterns, reconciling queue depth) to JSON and writes to clipboard; displayed as a collapsible code block on the dashboard for visual inspection; useful for SREs sharing cluster state in incident tickets without granting external access (issue #720)
- 🔲 SRE dashboard in-session health sparkline: per-RGD and per-cluster health trend chart on Overview SRE dashboard — tracks % ready / % error+degraded across the current browser session; rendered as lightweight SVG polylines with a legend and sample count; data accumulates from live poll results and resets on page reload; no persistence layer required (issue #712)
- 🔲 Stuck-reconciling escalation banner: persistent banner on Instance detail page when an instance has been IN_PROGRESS for >5 minutes; shows elapsed time, current conditions, and a kubectl patch command to force-reconcile or investigate; banner auto-dismisses when the instance transitions to a terminal health state; mirrors the Terminating banner UX pattern from spec 031 (issue #711)
