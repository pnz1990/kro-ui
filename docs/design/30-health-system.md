...
- 🔲 30.7 — Self-improvement: skills_delta=0 for 10 consecutive scans means the health system surface is accumulating undiscovered failure modes — the health system is the most-complex surface in kro-ui (6-state model, 5 display components, 3 aggregation layers) but `/otherness.learn` has not added a health-system-specific skill in any session; the absence of skills growth IS a health signal for the development system: when skills_delta=0 and features keep shipping, the agents are reasoning from first principles on each health-system PR rather than accumulating domain knowledge; add a Zone 1 obligation: when any health system feature PR is opened, the implementing agent MUST consult `~/.otherness/agents/skills/` for any skill covering health state computation, polling, or condition aggregation and record the skill name in the PR description; if no relevant skill exists, the agent MUST create one after the PR merges — this converts each health-system PR into a learning event rather than a one-off implementation
