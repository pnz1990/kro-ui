...
- 🔲 30.6 — Honesty pressure lens: health metrics are displayed but operators cannot verify the health signal is accurate — kro-ui shows GREEN health chips for instances with no available readiness information (no conditions loaded, degraded state not yet computed); a donation reviewer doing a live demo on a fresh cluster may see confident GREEN chips on instances that have not yet been checked; add a "data freshness" indicator on health chips: if the instance data was fetched >30s ago without a live confirmation, render the chip with a faded opacity + "as of Xs ago" tooltip; this converts the health display from a static badge to a time-stamped signal — consistent with WCAG 2.1 SC 1.3.3 (not relying solely on sensory characteristics) and honest about staleness
