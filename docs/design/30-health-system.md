...
- 🔲 Live health state change announcements: the instance detail page polls every 5s and updates `HealthPill` / `HealthChip` components silently; screen reader users who rely on `aria-live` regions receive no announcement when an instance transitions from reconciling→ready or error→degraded; add an `aria-live="polite"` region in the InstanceDetail page that announces health state changes (but only on transition, not every poll cycle — use a ref to track previous state); WCAG 2.1 SC 4.1.3 (Status Messages) requires status updates to be programmatically determinable
