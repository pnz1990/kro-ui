...
- 🔲 Health state legend / onboarding tooltip: a new operator who sees a reconciling (amber ↻) badge for the first time has no in-app explanation of what "reconciling" means vs "degraded" vs "error"; add a `?` icon or hover tooltip on health chip group headers (OverviewHealthBar, FleetMatrix, InstanceTable) that opens a compact legend explaining all 6 states and their resolution path; this is a **visibility + onboarding** gap — the health model is correct but opaque to anyone who hasn't read the docs
- 🔲 Fleet-wide reconciling count on Overview: the Overview SRE dashboard and Fleet matrix show per-cluster reconciling counts, but there is no cross-cluster total; an SRE managing a 10-cluster fleet cannot answer "how many instances across all clusters are currently reconciling?" in a single glance; add a `totalReconciling` field to the fleet summary response and surface it as a banner or stat chip on the Fleet page; currently requires clicking into each cluster card to add them up manually — a **visibility** gap at fleet scale
