...
- 🔲 Reliability: Lighthouse performance threshold was lowered from 50→45 (PR #741) — the threshold should rise now that code-splitting (PR #612) has landed; a 45 threshold means a 4.1MB PNG logo will not be caught by CI even though it caused a 36.8s LCP (documented AGENTS.md anti-pattern #661); raise the threshold to 60 after investigating the root-cause score drop; add a Lighthouse regression comment step to the CI workflow (issue #717) so any future score drops are visible on the PR that caused them (date: 2026-04-23)
- 🔲 Visibility: the `HEALTH_STATE_ICON` map in `format.ts` is the canonical single source of truth for health state representation but it has no visual regression test — if an icon is accidentally replaced with `?` or `undefined` the change would pass CI and unit tests; add a snapshot test that serializes the full `HEALTH_STATE_ICON` map and asserts it does not change without a deliberate update; this is the minimum safety net for the most user-visible correctness property in kro-ui (date: 2026-04-23)
- 🔲 Onboarding: operators new to kro do not know what the 6 health states mean without reading docs — the health chips show icons (✓/✗/⚠/↻/…/?) but there is no legend visible anywhere on the Overview or Fleet pages; the AGENTS.md anti-pattern table says "DAG legend" shipped in PR #TBD but instance health state legend has no equivalent; add a dismissible "Health states" legend tooltip on the Overview HealthBar that expands on click to show the 6 states with their icons, names, and one-line descriptions; this is the minimum onboarding aid for the primary signal in kro-ui (date: 2026-04-23)
- 🔲 Self-improvement: the `translateApiError()` function in `format.ts` has no test coverage for kro-specific error messages — it has generic HTTP status translations but kro surfaces structured error messages in `.status.message` that include phrases like "CEL expression error", "schema validation failed", "dependency not ready"; add a unit test suite with ≥8 kro-specific error message patterns and assert they produce human-readable translations; without this, a kro API change that modifies error message format will silently produce "Unknown error" throughout the UI (date: 2026-04-23)
