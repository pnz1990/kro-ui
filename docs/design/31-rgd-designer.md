...
- ✅ Designer: E2E journey for CEL expression linter — Playwright journey 091 covers: load `/author`, add a resource, expand advanced options, add a readyWhen row, type unclosed string literal, assert `role="alert"` error hint appears, clear expression, assert hint disappears, valid expression produces no hint; registered in chunk-9 (`testMatch` includes `091`); 6 steps, no `waitForTimeout` (spec issue-744, 🔲→✅ 2026-04)
