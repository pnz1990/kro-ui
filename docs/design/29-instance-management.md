...
- ✅ 29.2 — Live DAG node selection keyboard shortcut: pressing `/` opens a node-search box that filters nodes by label/kind/ID; Enter selects the top match; Escape closes; non-matching nodes dimmed to 0.25 opacity; `role="searchbox"` + `aria-label` for WCAG 2.1 SC 4.1.2; O7 guard prevents `/` when a text input is already focused (spec issue-771, 2026-04)
