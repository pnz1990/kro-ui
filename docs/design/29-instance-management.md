...
- 🔲 Instance management: instance health silence detection — an instance can be stuck in `reconciling` state for hours with no transition; the UI polls every 5s and re-renders the same state with no escalation; the deletion debugger escalates after 5 minutes for *terminating* instances (PR #290) but no equivalent exists for *reconciling* instances that are not terminating; add a "stuck reconciling" banner that triggers when an instance has been in `reconciling` state continuously for >10 minutes (detected via instance `creationTimestamp` vs current time, or via the last observed state-transition in conditions); the banner should show the elapsed time and suggest `kubectl describe` as a first debugging step (2026-04)
- 🔲 Instance management: instance count per namespace summary — the `/instances` page shows instances from all namespaces but has no summary of "how many instances per namespace"; an operator managing 20 namespaces cannot see at a glance which namespace has the most activity or the most errors; add a namespace summary row or collapsible header to the `/instances` table that shows `N instances (M errors)` per namespace when more than 1 namespace is present; this is the product-level equivalent of the loop's "session output summary" gap — both are about making aggregated state visible without requiring manual counting (2026-04)
- 🔲 Instance management: live DAG polling opt-out — the live DAG polls at 5s unconditionally; in a busy cluster with many instances or many observers, this produces unnecessary API load; an operator who opened an instance detail page and left their laptop open overnight is generating thousands of list calls; add a "Pause live updates" toggle to the instance detail page that stops polling when the tab is backgrounded (using `document.visibilityState`) and resumes when the tab is foregrounded; this is also an accessibility improvement — continuous DOM updates can trigger screen reader announcements that interrupt users; the aria-live region in InstanceDetail (shipped PR #670) already gates announcements on state transitions, but the DOM still mutates on every poll (2026-04)
