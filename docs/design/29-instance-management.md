...
- ✅ 29.4 — Per-node reconciliation duration badge: each live DAG node detail panel shows "Xm in state" next to the live state badge; data source is `conditions[type=Ready].lastTransitionTime` on the child resource; `stateEnteredAt` field added to `NodeStateEntry`; `formatAge()` utility renders the age; absent when child has no conditions (spec issue-773, 🔲→✅ 2026-04)
