...
- 🔲 YAML diff line-level highlighting: the raw YAML diff panel in `RevisionsTab` renders two `KroCodeBlock` components side-by-side with no indication of which lines differ between revision A and revision B; a user comparing two revisions must read both YAML blocks manually to find the change; add a pure-TypeScript Myers diff on the YAML string lines (no new dependency) and render added lines with a green left-border and removed lines with a red left-border within each column; this is what makes the diff view actually useful rather than showing two disconnected code dumps; the DAG diff (RGDDiffView) shows node-level changes, but the YAML panel should show the raw line-level delta for operators who need to see exactly what field changed
