...
- ✅ 28.2 — Catalog RGD complexity score: `complexity_score = resources.length + (chaining_count × 2) + (forEach_count × 3)` numeric badge on each catalog card; default sort "Most complex" (descending, configurable); `computeComplexityScore` + `countChainingReferences` pure functions in `@/lib/catalog.ts` with 9 unit tests; no new backend required (spec issue-768, 🔲→✅ 2026-04)
- ✅ 28.3 — Self-improvement gate for DAG: Zone 1 **O4** obligation added — before opening a PR for any DAG feature, the implementing agent MUST read `web/src/components/DAGGraph.tsx` and `web/src/lib/dag.ts` and enumerate the 5 most recent DAG-related anti-pattern entries in `AGENTS.md`; no DAG PR may be opened without this check recorded in the PR description; this is the frame-lock prevention mechanism for the highest-risk component (spec issue-769, 🔲→✅ 2026-04)
- ✅ 28.4 — Visibility: RGD display "advancement signal" — `docs/aide/product-changelog.md` added: 10-row rolling table of most recently shipped user-visible features across docs 28–31; columns: Feature | Docs area | Date shipped | PR; SM updates after each batch; distinct from metrics.md (velocity) and loop-health.md (system health) (spec issue-770, 🔲→✅ 2026-04)
