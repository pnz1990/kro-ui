...
| `skills_delta=0` for 8+ consecutive scans — learning is visibly stalled | doc 27 §27.24 | The metrics table shows `skills_delta=0` across all 8 reconstructed rows; the skills library has 17 files as of 2026-04-23 but no new skill has been added since at least batch 51 (2026-04-20); `/otherness.learn` has not run in any visible session; the pressure lens "Is the self-improvement real?" reads as NO — the loop is executing but not learning |
...
| Onboarding time-to-value never measured — "1 hour" claim is untested | doc 27 §27.27 | Item 27.27 says "a new project should be running its first autonomous batch within 1 hour"; there is no measurement of how long this actually takes; a new project setup requires: install otherness, run /otherness.onboard, configure otherness-config.yaml, first batch runs; if any step fails silently the operator has no visibility; add a `docs/aide/onboarding-time.md` that records the actual elapsed time for the last 3 new-project onboardings (if any) and the specific manual steps that were required; without this, "1 hour" is aspiration not fact
