...
| `/otherness.learn` not running | doc 27 §27.69 | `skills_count` stuck at 12 across all measured batches; skills library not growing; loop not getting smarter |
...
| `skills_delta=0` for 8+ consecutive scans — learning is visibly stalled | doc 27 §27.69 | The metrics table shows `skills_delta=0` across all 8 reconstructed rows; the skills library has 17 files as of 2026-04-23 but no new skill has been added since at least batch 51 (2026-04-20); `/otherness.learn` has not run in any visible session; the pressure lens "Is the self-improvement real?" reads as NO — the loop is executing but not learning |
...
| Meaningful PR not defined — GREEN health earned by housekeeping PRs | doc 27 §27.S5 | No `meaningful_pr_pattern` in otherness-config.yaml; a docs-only PR counts identical to `feat: new instance panel` in the metrics table; busywork-spiral detection fires on total PRs, not feature PRs — the GREEN signal is structurally un-falsifiable by housekeeping-only batches |
| Report issue comment stream too verbose — health unreadable at a glance | doc 27 §27.S4 | SM and vibe-vision-auto post paragraph-heavy technical comments; a human opening issue #439 today cannot assess loop health in <10 seconds; 5-row table (Health / Last feature PR / Next queue item / Open blockers / E2E status) would make it scannable |
| Onboarding self-test missing — new project setup quality unverified | doc 27 §27.S3 | `/otherness.onboard` generates docs but no automated lint check validates completeness; `make otherness-onboard-lint` would fail fast on missing required keys, absent vision.md, or unresolvable report_issue — instead of silent errors in session 10 |
| Self-improvement gate not enforced — skills_delta=0 for 9+ batches with no forced learn | doc 27 §27.S1 | `learn_stagnation_threshold` not in otherness-config.yaml; SM §4b has no rule to auto-insert /otherness.learn when skills_delta=0 for ≥3 consecutive batches; the stagnation continues passively |
